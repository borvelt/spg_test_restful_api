...
export class PostContorllerController {
  constructor(
  ...
  @post('/posts', {
    responses: {
      '200': {
        ...
        content: {'application/json': {schema: getModelSchemaRef(Post), exclude: ['id', 'createdAt', 'updatedAt', 'userId', 'comments', 'reactions',], }},
      },
    },
  })
  async create(
  ...
  async updateAll(
    @requestBody({
      content: {
        'application/json': {
          schema: getModelSchemaRef(Post, {partial: true, exclude: ['id', 'createdAt', 'updatedAt', 'userId', 'comments', 'reactions',]}),
        },
      },
    })
    post: Post,
